...
from aswaxs_live.paths import PACKAGE_DIR
...
def poni_geometry_metadata(poni_file: str | Path) -> dict[str, object]:
    """Return detector geometry metadata from a pyFAI PONI file.

    ``dist``/``poni1``/``poni2`` are the native pyFAI values in meters.  The
    ``beam_center_*_px`` values are the image-space center coordinates used by
    most users when checking a calibration.  For tilted geometries pyFAI's
    Fit2D conversion is preferred when available.
    """
    ai = load_poni(str(Path(poni_file).expanduser().resolve()))
    distance_m = _optional_float(getattr(ai, "dist", None))
    metadata: dict[str, object] = {
        "poni_file": str(Path(poni_file).expanduser().resolve()),
        "sample_detector_distance_m": distance_m,
        "sample_detector_distance_mm": None if distance_m is None else distance_m * 1000.0,
        "pyfai_dist_m": distance_m,
        "pyfai_poni1_m": _optional_float(getattr(ai, "poni1", None)),
        "pyfai_poni2_m": _optional_float(getattr(ai, "poni2", None)),
        "pyfai_rot1_rad": _optional_float(getattr(ai, "rot1", None)),
        "pyfai_rot2_rad": _optional_float(getattr(ai, "rot2", None)),
        "pyfai_rot3_rad": _optional_float(getattr(ai, "rot3", None)),
        "pixel1_m": _optional_float(getattr(ai, "pixel1", None)),
        "pixel2_m": _optional_float(getattr(ai, "pixel2", None)),
        "geometry_source": "PONI",
        "beam_center_coordinate_system": "detector pixels; x follows detector column axis, y follows detector row axis",
    }
    metadata.update(_fit2d_geometry_metadata(ai))
    if metadata.get("beam_center_x_px") is None or metadata.get("beam_center_y_px") is None:
        center_x, center_y = _beam_center_from_poni_pixels(ai)
        metadata.setdefault("beam_center_x_px", center_x)
        metadata.setdefault("beam_center_y_px", center_y)
    return {key: value for key, value in metadata.items() if value is not None}


def _fit2d_geometry_metadata(ai: object) -> dict[str, object]:
    try:
        fit2d = ai.getFit2D()
    except Exception:
        return {}
    return {
        "beam_center_x_px": _fit2d_value(fit2d, "centerX"),
        "beam_center_y_px": _fit2d_value(fit2d, "centerY"),
        "fit2d_direct_distance_mm": _fit2d_value(fit2d, "directDist"),
        "fit2d_tilt_deg": _fit2d_value(fit2d, "tilt"),
        "fit2d_tilt_plane_rotation_deg": _fit2d_value(fit2d, "tiltPlanRotation"),
    }


def _fit2d_value(fit2d: object, name: str) -> float | None:
    if isinstance(fit2d, dict):
        return _optional_float(fit2d.get(name))
    return _optional_float(getattr(fit2d, name, None))


def _beam_center_from_poni_pixels(ai: object) -> tuple[float | None, float | None]:
    poni1 = _optional_float(getattr(ai, "poni1", None))
    poni2 = _optional_float(getattr(ai, "poni2", None))
    pixel1 = _optional_float(getattr(ai, "pixel1", None))
    pixel2 = _optional_float(getattr(ai, "pixel2", None))
    center_y = poni1 / pixel1 if poni1 is not None and pixel1 and pixel1 > 0 else None
    center_x = poni2 / pixel2 if poni2 is not None and pixel2 and pixel2 > 0 else None
...
def _write_outputs(
    ...
    metadata = {
        ...
        "unit": args.unit,
        "frame": args.frame,
        **poni_geometry_metadata(args.poni),
    }
    header = "\n".join(
        [
            "ASWAXS 1D reduction",
