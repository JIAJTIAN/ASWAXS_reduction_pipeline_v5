...
def reduce_manifest_frames(
    ...
    for idx, item in enumerate(items, start=1):
        ...
        source_file_bytes = item.path.stat().st_size
...
def final_outputs_for_recipe(
    ...
    *,
    ...
    records = [build_final_record(energy_index, lookup, recipe_args, gc_reference_file) for energy_index in energy_indices]
...
def write_final_sample_outputs(averages: list[GroupAverage], args: argparse.Namespace, output_dir: Path) -> list[FinalOutput]:
    ...
    for recipe in extraction_recipes(args):
        outputs.extend(final_outputs_for_recipe(averages, args, output_dir, recipe, write_text=True))
    return outputs
